...
auto v1::TMP102Controller::getTemperatureC() -> Result< float >
{
	...
	if( read( kTemperatureRegister, data.data(), data.size() ) > 0 )
	{
		std::uint8_t config[ 2 ]{};
		if( read( kConfigurationRegister, config, 2 ) <= 0 ) return std::unexpected( utils::ErrorCode::FAILED_TO_READ );
...
{
	auto celsius = getTemperatureC();
	if( celsius.has_value() ) return utils::celsiusToFahrenheit( celsius.value() );
	return celsius;
}
...
{
	std::uint8_t config[ 2 ]{};
	if( read( kConfigurationRegister, config, 2 ) > 0 )
	{
...
{
	// TMP102 alert status can often be read from ALERT pin; here we simulate register-based
	std::uint8_t config[ 2 ]{};
	if( read( kConfigurationRegister, config, 2 ) > 0 )
	{
		std::bitset< 8 > lsb( config[ 1 ] );
